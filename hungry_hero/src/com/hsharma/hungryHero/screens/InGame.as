package com.hsharma.hungryHero.screens {
	...
	//Esta clase contiene el código completo de la mecánica del juego.
	...
	public class InGame extends Sprite{
		...
		/** Obstáculos en la piscina con un tope máximo para la reutilización de los elementos. */		
		...
		//Dibuja los elementos del juego - de fondo, héroe, partículas, botón de pausa, el botón de inicio y artículos (en la piscina).
		...
		public function initialize():void{
			...
			disposeTemporarily();
			this.visible = true;
			this.addEventListener(Event.ENTER_FRAME, calculateElapsed);	// Calcular el tiempo transcurrido.
			
			if (!Sounds.muted) Sounds.sndBgGame.play(0, 999);// Play screen background music.
			
			gameArea = new Rectangle(0, 100, stage.stageWidth, stage.stageHeight - 250);// Define game area.
			lives = GameConstants.HERO_LIVES;// Define lives.
			...
			// Restablecer estado del juego a idle.
		...
		//Deseche la pantalla temporalmente.
		...
		//El botón de inicio haga clic en.
		private function onStartButtonClick(event:Event):void{
			// Reproducir sonido del café de clic de botón.
			if (!Sounds.muted) Sounds.sndCoffee.play();
			startButton.visible = false;// Hide start button.
			pauseButton.visible = true;// Show pause button since the game is started.
			launchHero();// Launch hero.
		}
			private function launchHero():void{
				playerSpeed = 0;	// Touch interaction
				this.addEventListener(TouchEvent.TOUCH, onTouch);	// Game tick
		...
		private function onGameTick(event:Event):void{
			if (!gamePaused){// Si no se detuvo, marque el juego.
				if (isNaN(touchX)){// Si no touch coordenadas, restablezca touchX y delicado (para dispositivos con pantalla táctil).
					touchX = stage.stageWidth * 0.5;
					touchY = stage.stageHeight * 0.5;
				}
				// Si la representación de hardware, establezca x del emisor de partículas y.
				if (isHardwareRendering){
				...
				switch(gameState){
					case  GameConstants.GAME_STATE_IDLE://Antes del inicio del juego.
						// Quítese.
					...
					// Cuando juego está en marcha.
					case GameConstants.GAME_STATE_FLYING:
						
						// Si tomamos café, volar más rápido por un tiempo.
						if (coffee > 0){
							playerSpeed += (GameConstants.HERO_MAX_SPEED - playerSpeed) * 0.2;
						}
						
						// Si no es afectada por obstáculos, volar con normalidad.
						if (hitObstacle <= 0){
							hero.y -= (hero.y - touchY) * 0.1;
							
							// Si el héroe está volando muy rápido, crear un efecto de viento y campo de fuerza alrededor espectáculo héroe.
							if (playerSpeed > GameConstants.HERO_MIN_SPEED + 100){
								createWindForce();
								// Animar héroe más rápido.
						...
						}else{// Golpeado por obstáculos
							if (coffee <= 0){
								// Reproducir animación héroe por hit obstáculo.
								if (hero.state != GameConstants.HERO_STATE_HIT){
									hero.state = GameConstants.HERO_STATE_HIT;
								}
								
								// Mover héroe al centro de la pantalla.
						...
						// Si tenemos un hongo, reducir el valor de la potencia.
						if (mushroom > 0) mushroom -= elapsed;
						
						// Si tenemos un café, reducir el valor de la potencia.
						...
						// Guarde x actual del héroe y posiciones y (necesaria para las animaciones más abajo).
						heroX = hero.x;
						heroY = hero.y;
						...
						// Ajustar la velocidad del fondo basado en la velocidad del héroe.
						bg.speed = playerSpeed * elapsed;
		...
		// Animar obstaculos señalados párrafo la animación.
		private function animateObstacles():void{
			if (!gamePaused){
				...
				for (var i:uint = 0; i < obstaclesToAnimateLength ; i ++){
					...
					//Si la distancia es aún mayor que 0, seguir reduciendo su valor, sin mover el obstáculo.
					if (obstacleToTrack.distance > 0 ){
						obstacleToTrack.distance -= playerSpeed * elapsed;  
					}else{
						// De lo contrario, mueva el obstáculo basado en la velocidad del héroe, y comprobar si se le pega.
						// Retire la búsqueda de señal.
						if (obstacleToTrack.lookOut == true ){
							obstacleToTrack.lookOut = false;
						}
						
						// Mueva el obstáculo basado en la velocidad del héroe.
						obstacleToTrack.x -= (playerSpeed + obstacleToTrack.speed) * elapsed; 
					}
					
					// Si el obstáculo pasa más allá de la pantalla, eliminarlo.
					...
					// Detección de colisiones - Compruebe si choca héroe con cualquier obstáculo.
					...
					if (gameState == GameConstants.GAME_STATE_OVER){//Si el juego ha terminado, retire el obstáculo.
						disposeObstacleTemporarily(i, obstacleToTrack);
					}
				}
			}
		...
		private function endGame():void{
			this.x = 0;
			this.y = 0;
			// Set Game Over estado para todos los obstáculos y objetos pueden retirarse.
			gameState = GameConstants.GAME_STATE_OVER;
		}
		
		//Game Over - llama cuando el héroe se cae de la pantalla y cuando se debe mostrar Game Over datos.
		...
		private function shakeAnimation(event:Event):void{
			// Animar efecto terremoto, sacudiendo la cámara un poco hacia los lados y hacia arriba y abajo.
			if (cameraShake > 0){
				cameraShake -= 0.1;
				this.x = int(Math.random() * cameraShake - cameraShake * 0.5); // Agite izquierda derecha al azar.
				this.y = int(Math.random() * cameraShake - cameraShake * 0.5); // sacudir arriba abajo al azar.
			}else if (x != 0) {
				// Si el valor es 0 sacudida, restablezca la etapa posterior a la normalidad. Cambiar a la posición inicial.
				this.x = 0;
				this.y = 0;
			}
		}
		
		//Crear una partícula de comer cuando se recoge un artículo.
		private function createEatParticle(itemToTrack:Item, count:int = 2):void{
			var eatParticleToTrack:Particle;
			while (count > 0){
				count--;
				// Crear comer objeto de partículas.
				eatParticleToTrack = eatParticlesPool.checkOut();
				
				if (eatParticleToTrack){
					// Ajuste la posición del objeto de partículas con un desplazamiento aleatorio.
					eatParticleToTrack.x = itemToTrack.x + Math.random() * 40 - 20;
					eatParticleToTrack.y = itemToTrack.y - Math.random() * 40;
					// Ajuste la velocidad de un objeto de partículas.
					eatParticleToTrack.speedY = Math.random() * 10 - 5;
					eatParticleToTrack.speedX = Math.random() * 2 + 1;
					// Ajuste la velocidad de giro del objeto de partículas.
					eatParticleToTrack.spin = Math.random() * 20 - 5;
					// Ajuste la escala de la partícula comer.
					eatParticleToTrack.scaleX = eatParticleToTrack.scaleY = Math.random() * 0.3 + 0.3;
					// Animar la partícula comer.
					eatParticlesToAnimate[eatParticlesToAnimateLength++] = eatParticleToTrack;
				}
			}
		}
		
		//Animación de las partículas de viento que están marcados animatable.
		private function animateWindParticles():void{
			var windToTrack:Particle;
			for(var i:uint = 0;i < windParticlesToAnimateLength;i++){
				windToTrack = windParticlesToAnimate[i];// Cree partículas del viento.
				
				if (windToTrack){
					windToTrack.x -= 100 * windToTrack.scaleX; // Mueva el viento en función de su escala.
					if (windToTrack.x < -windToTrack.width || gameState == GameConstants.GAME_STATE_OVER){// Si la partícula viento tira fuera de la pantalla, retírelo.
		...
		// Animar las partículas come marcados animatable.
		private function animateEatParticles():void{
			...
			for(var i:uint = 0;i < eatParticlesToAnimateLength;i++){
				...
				if (eatParticleToTrack){
					...
					eatParticleToTrack.scaleY = eatParticleToTrack.scaleX;// Hacer la partícula comer se hacen más pequeños.
					eatParticleToTrack.y -= eatParticleToTrack.speedY; //Mueve horizontalmente sobre la base de speedX.
					eatParticleToTrack.speedY -= eatParticleToTrack.speedY * 0.2;//Reduzca la velocidad horizontal.
					eatParticleToTrack.x += eatParticleToTrack.speedX;//Mueva verticalmente sobre la base de Speedy.
					eatParticleToTrack.speedX--; //Reduzca la velocidad vertical.

					eatParticleToTrack.rotation += deg2rad(eatParticleToTrack.spin); //Gire la partícula comer basado en giro.
					eatParticleToTrack.spin *= 1.1; // Aumentar la velocidad de giro.
					
					if (eatParticleToTrack.scaleY <= 0.02){// Si la partícula comer es lo suficientemente pequeño, retírelo.
		...
		//El juego sobre la pantalla se desvaneció.
		...
		private function calculateElapsed(event:Event):void{
			timePrevious = timeCurrent;// Ajuste la hora actual como la vez anterior.
			timeCurrent = getTimer(); // Obtenga teh nuevo tiempo actual.
			elapsed = (timeCurrent - timePrevious) * 0.001; // Calculate el tiempo que se tarda para una frame para pasar, en milisegundos.
		}
	}
}